...
DROP TABLE IF EXISTS dict_event_type;
...
CREATE INDEX IF NOT EXISTS fki_fk_reviews_film_id
    ON reviews
        (film_id ASC NULLS LAST);

CREATE INDEX IF NOT EXISTS fki_fk_reviews_user_id
    ON reviews
        (user_id ASC NULLS LAST);
...
(
    ...
    CONSTRAINT review_score_uniq_con UNIQUE (review_id, user_id)
);

CREATE INDEX IF NOT EXISTS fki_fk_review_scores_review_id
    ON review_scores
        (review_id ASC NULLS LAST);

CREATE INDEX IF NOT EXISTS fki_fk_review_scores_user_id
    ON review_scores
        (user_id ASC NULLS LAST);
...
CREATE TABLE IF NOT EXISTS dict_operation
(
    id   smallint  NOT NULL,
    name character varying(30) NOT NULL,
    PRIMARY KEY (id)
...
--     event_timestamp timestamp without time zone,
    ...
    event_type int NOT NULL REFERENCES dict_event_type (id) ON DELETE CASCADE,
    operation int NOT NULL REFERENCES dict_operation (id) ON DELETE CASCADE,
    PRIMARY KEY (id)
    );
